...
export function Pre({ title, ...props }: ComponentProps<'pre'>) {
    ...
    return (
        ...
        >
            {title && (
                <div className="text-sm text-muted-foreground bg-muted pl-4 pr-12 py-2 border-b z-[2]">{title}</div>
            )}
...
function CopyButton({ onCopy }: { onCopy: () => void }) {
    ...
    return (
        <button
            ...
            onClick={onClick}
        >
            <CheckIcon className={cn('absolute w-3.5 h-3.5 transition-transform', !checked && 'scale-0 rotate-90')} />

            <CopyIcon className={cn('w-3.5 h-3.5 transition-transform', checked && 'scale-0 rotate-90')} />
        </button>
    )
}
